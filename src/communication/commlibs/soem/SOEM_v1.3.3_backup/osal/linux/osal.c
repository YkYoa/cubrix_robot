...
int osal_thread_create_rt_with_cpu(void *thandle, int stacksize, void *func, void *param, int cpu_num)
{
      int                  ret;
   pthread_attr_t       attr;
   struct sched_param   schparam;
   ...
   int s;
   s = pthread_setaffinity_np(*threadp, sizeof(cpu_set_t), &cpuset);
   if (s != 0)
      printf("Error in pthread_setaffinity_np: %d\n", s);
      // handle_error_en(s, "pthread_setaffinity_np");
   else
      printf("Success in pthread_setaffinity_np\n");

   /* Check the actual affinity mask assigned to the thread. */

   s = pthread_getaffinity_np(*threadp, sizeof(cpu_set_t), &cpuset);
   if (s != 0)
      printf("Error in pthread_getaffinity_np: %d\n", s);
      // handle_error_en(s, "pthread_getaffinity_np");
   else
      printf("Success in pthread_getaffinity_np\n");
